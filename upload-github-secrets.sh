...
# Usage:
#   ./upload-github-secrets.sh -f mailu.env -r osmarbetancourt/mailu-server -e production

set -euo pipefail
shopt -s extglob  # enable +() patterns used for trimming

ENV_FILE="mailu.env"
REPO="osmarbetancourt/mailu-server"
...
Options:
  -f  Path to env file (default: mailu.env)
  -r  GitHub repo in owner/name format (default: osmarbetancourt/mailu-server)
